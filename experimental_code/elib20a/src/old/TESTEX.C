...
int main()
{
  ...
  drawText(&picBuf, "A",154,236, 1, COL_WHITE);
  ...
  fr2  = newImage();
  fillPicBuf(COL_WHITE);
  circlefill(picBuf  ,160,240,60, COL_RED2);
  drawText(&picBuf, "A",154,236, 1, COL_WHITE);
  circlefill(picBuf  ,480,240,60, COL_BLUE1);
  drawText(&picBuf, "B",474,236, 1, COL_WHITE);
  uploadImage(fr2);
 ...
 /* set up the movie, and run it */
  mymovie = initMovie(5);
  setMovie(mymovie, 0, fr0, 180);
  setMovie(mymovie, 1, fr1, 89);
  setMovie(mymovie, 2, fr2, 1);
  setMovie(mymovie, 3, fr1, 180);
  setMovie(mymovie, 4, fr0, 180);
  clearScreen(0);
  data = runMovie(mymovie, UNTIL_RESPONSE, 1);
  clearScreen(0);  
