...
int main()
{
  ...
  UseDIO24 = 0;
  coltable = fopen("table1","r");
  for(i=0; i<1024; i++){
    fscanf(coltable," %d %d %d\n", &c1, &c2, &c3);  
    pal[i] = c1+32*(c2 + 32*c3);
  }
  fclose(coltable);
  /* must be called to intialize the Movie package */
  setup(640, 480, 512, 256, 127+256*(127 +256*127), 120, 15, "fonts.lst");
  makePalette(CLEAR);
